...
public class SinglyLinkedList<E> implements List<E>, Iterable<E> {
    ...
    private int count(Node<E> current, E fromElement, E toElement, boolean found) {
        if (current == null) {
            return 0;
        }
        if (found == true) {
            if (current.value.equals(toElement)) {
                return 1;
            } else if (!current.value.equals(toElement)) {
                return 1 + count(current.next, fromElement, toElement, true);
            }
        } else if (current.value.equals(fromElement)) {
            return 1 + count(current.next, fromElement, toElement, true);
        } 
        return count(current.next, fromElement, toElement, false);
    }
